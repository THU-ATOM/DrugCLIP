...
import numpy as np
import torch
from Bio.PDB import Chain, PDBParser, is_aa
...
def get_binding_pockets(biopy_chain, liglist):
    ...
    for n, lig in liglist:
        lig_coord = np.array(
            [i.coord for i in lig.get_atoms() if i.element != "H"]
        )
        tmp_chain = Chain.Chain("A")
    ...
    for d in os.listdir(dirs):
        if ".pdb" in d and "_clean" not in d:
            try:
                ...
                tmp_chain, liglist = extract_lig_recpt(
                    model, re.search(r"_.*\.", d)[0][1:-1]
                )
...
def main(args):
    ...
    # Print args
    logger.info(f"encode pockets args: {args}")
    ...
    logger.info(f"pocket_names: {pocket_names}")

    # save names and reps together to pickle file

    with open(
        os.path.join(args.pocket_dir, "pocket_reps_project.pkl"), "wb"
    ) as f:
